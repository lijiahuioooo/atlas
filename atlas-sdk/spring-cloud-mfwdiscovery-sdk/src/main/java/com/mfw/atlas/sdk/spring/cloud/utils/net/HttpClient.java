package com.mfw.atlas.sdk.spring.cloud.utils.net;/*
...
import com.mfw.atlas.sdk.spring.cloud.utils.IoUtils;
...
public class HttpClient {
    ...
    public static HttpResult request(String url, List<String> headers, Map<String, String> paramValues, String encoding, String method) {
        ...
        try {
            ...
            try {
                if (conn != null) {
                    LogUtils.NAMING_LOGGER.warn("failed to request " + conn.getURL() + " from "
                        + InetAddress.getByName(conn.getURL().getHost()).getHostAddress());
                }
            } catch (Exception e1) {
                LogUtils.NAMING_LOGGER.error("[NA] failed to request ", e1);
                //ignore
            }

            LogUtils.NAMING_LOGGER.error("[NA] failed to request ", e);
